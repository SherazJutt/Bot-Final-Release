...
class FindTextClass { ;// Class Begin
    ...
    FindText(OutputX := "", OutputY := ""
        ...
        sx := x, sy := y, sw := w, sh := h
            ...
            , ini := { sx: sx, sy: sy, sw: sw, sh: sh, zx: zx, zy: zy, mode: mode, bits: bits, ss: ss.Ptr, s1: s1.Ptr, s0: s0.Ptr, err1: err1, err0: err0, allpos_max: allpos_max, zoomW: zoomW, zoomH: zoomH }
        loop 2 {
  ...
  return ok;
    ...
    BindWindow(bind_id := 0, bind_mode := 0, get_id := 0, get_mode := 0) {
        ...
        if (get_mode)
            return bind.mode
        if (bind_id) {
            bind_id := this.Floor(bind_id), bind.id := bind_id
    ...
    PixelCount(x1 := 0, y1 := 0, x2 := 0, y2 := 0, ColorID := "", Variation := 0, ScreenShot := 1) {
        ...
        , ini := { bits: bits, ss: 0, s1: s1.Ptr, s0: s0.Ptr, err1: 0, err0: 0, allpos_max: 0, zoomW: 1, zoomH: 1 }
        , n := this.Floor(Variation), text := Format("##{:06X}$0/0", n << 16 | n << 8 | n)
        loop parse, ColorID, "|"
            if (v := Trim(A_LoopField)) != ""
                text .= "/" v
    ...
    Gui(cmd, arg1 := "", args*) {
        ...
        switch cmd {
            ...
            case "ScreenShot":
                ...
                CoordMode "ToolTip"
                this.ToolTip(Lang["s9"], , 0, , { bgcolor: "Yellow", color: "Red", size: 48, bold: "bold", trans: 200, timeout: 0.2 })
                return
            ...
            case "Color2Two", "ColorPos2Two":
                ...
                n := this.Floor(FindText_Capture[UsePos ? "Similar2" : "Similar1"].Value)
