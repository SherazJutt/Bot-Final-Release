...
TdCheck() {
    ErrorLevel := !PixelSearch(&FoundX, &FoundY, 897, 647, 916, 677, 0xC0F712)
    If (ErrorLevel = 0) {
        Sleep(1000)
...
PlayHuntRace() {
    ...
    Loop 200 {
        ...
        if (ok := FindText(&X, &Y, 1001, 621, 1079, 656, 0, 0, Text)) {
            ...
            if !CheckInternet() || !isGameRunning() {
                Break
            }
        }
    }
...
HuntNitroManagemnet() {
    ...
    ; almost end
    ErrorLevel := !PixelSearch(&FoundX, &FoundY, 668, 61, 800, 98, 0xFFFF00)
    If (ErrorLevel = 0) {
        Send("{ Space 1 }")
        Sleep(250)
        Send("{ Space 1 }")
    ...
    ; mid
    ErrorLevel := !PixelSearch(&FoundX, &FoundY, 554, 59, 667, 97, 0xFFFF00)
    If (ErrorLevel = 0) {
        Send("{ Space }")
        Sleep(1000)
        Send("{ Space }")
    ...
    ; start
    ErrorLevel := !PixelSearch(&FoundX, &FoundY, 437, 62, 555, 97, 0xFFFF00)
    If (ErrorLevel = 0) {
        Send("{ Space 1 }")
        Sleep(250)
        Send("{ Space 1 }")
...
HuntRewardsSkip() {
    ...
    Loop 30 {
        Text := "|<>*140$68.w0w7zsw0wzzz0D1zy7UDDzzs3kTzUs7Xzzy0w700D1s1sDkD1k01kw0S3y3kQ00SD07UzUw7003bU1sDwD1k00xs0S3r3kQ007w07Uxsw7zU1y01sDCD1zw0DU0S3nnkTz03s07UwQw7001z01sD3j1k00zk0S3kzkQ00DS07Uw7w7007XU1sD1z1k01sw0S3kDkQ00w707Uw3w700D1s1sD0T1s07UC0S3k7kTzVs3k7Uw0w7zsw0Q1s8"
        if (ok := FindText(&X, &Y, 1109, 634, 1187, 668, 0, 0, Text)) {
